...
if (!file.exists(session_path)) {
  stop("The session metadata file does not exist: ", session_path)
}
...
if (length(missing) > 0) {
  stop("sessions.csv lacks these required columns: ",
       paste(missing, collapse = ", "))
}
sessions <- sessions |>
  mutate(session_date = as.Date(session_date))
cat(sprintf("The script loaded %d sessions for %d patients.\n",
...
if (!is.null(scores_path)) {
  if (!file.exists(scores_path)) {
    warning("The clinical scores file does not exist: ", scores_path, ". The script skips it.")
  } else {
    scores <- read_csv(scores_path, show_col_types = FALSE)
    if (!"video" %in% names(scores)) {
      warning("The clinical scores CSV has no 'video' column. The script skips it.")
      scores <- NULL
    } else {
      score_cols <- setdiff(names(scores), "video")
      score_cols <- score_cols[map_lgl(score_cols, \(c) is.numeric(scores[[c]]))]
      if (length(score_cols) == 0) {
        warning("The clinical scores CSV has no numeric score columns. The script skips it.")
        scores <- NULL
      } else {
        cat(sprintf("The script loaded %d scores: %s.\n",
...
cat("The script joins the features with the session metadata.\n")
...
if (length(unmatched_feat) > 0) {
  warning(sprintf(
    "The session metadata does not contain these %d feature videos: %s.",
    length(unmatched_feat), paste(unmatched_feat, collapse = ", ")
  ))
}
if (length(unmatched_sess) > 0) {
  warning(sprintf(
    "The clinical features do not contain these %d session videos: %s.",
    length(unmatched_sess), paste(unmatched_sess, collapse = ", ")
  ))
}
if (length(matched) == 0) {
  stop("The join matched no videos between the features and sessions.")
}
cat(sprintf("The script matched %d videos across %d patients.\n",
            length(matched),
...
if (length(mean_cols) == 0) {
  stop("The script found no aggregated mean features.")
}
...
cat("The script computes deltas from baseline.\n")
...
for (pid in patients) {
  ...
  if (n_sessions < 2) {
    cat("    The script computes no deltas for a single session.\n")
  }
}

# --- Per-patient plots ---
cat("\nThe script generates per-patient plots.\n")
for (pid in patients) {
  feats <- select_top_features(summary_tbl, pid)
  if (length(feats) == 0) {
    cat(sprintf("  Patient %s has no variable features to plot. The script skips the plot.\n", pid))
    next
  }
