...
MAIN's batch ruling on `test-m2u4` phase 1, over the phase-1 table's **82 cases**, carried by the **34 amendments** below. Each row binds for the unit. The phase-1 prose that counts 80 is corrected in §8's closing note; `scripts/check_m2u4_suite_seed.py`, this intro and the roadmap agree on 82.
