...
**`archive/m3u3-test` (`7026e2c`) is retained unmerged and unscheduled** — `tests/test_r_qc_evidence.py`, 992 lines, diff-blind, red against `cc8a939`; its remaining cases are exactly the five cut groups. It is kept because it encodes contract requirement rather than an implementation, and because the five cut groups are 3D clinical metrics on the path to the eventual `../rehab` dashboard join. Reviving M3.3b starts there, against the frozen contract in `.agent/archive/contract-m3u3.md`.

**Both carrier spikes are deleted** — `wt/spike-m3u3-long` (`1097a4b`) and `wt/spike-m3u3-wide` (`d4ad6c3`). Every durable measurement they bought is recorded above and in the contract: 7/8 fault detectability each, wide's 55 → 125 column cost, and its inability to express per-metric status. What their code still held was the losing side of decisions this contract froze — long keyed by group where the contract keys by `metric_id`, wide gated estimates under policy — plus two changes ruled specifically forbidden, R-10's confidence-seam repair (`do not adopt`) and R-11's estimate gating, which broke four `tests/test_r_trajectory_kernel.py` cases. Executable code embodying a ruled-against design is quotable by a future agent, which is the hazard that retired `scripts/geometry_qualification.py` in M2.3 window 9.
