...
`(base / user_ref).resolve()` in `multicam.py` accepted `../` traversal from session.json manifest fields. **Rule:** after resolving a relative path, check it stays within its base (`str(resolved).startswith(str(base_resolved) + "/")`) before use. **Where:** `multicam.py:_safe_resolve`, `tests/test_multicam.py:test_*_path_traversal_*`.
