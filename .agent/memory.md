...
- 2026-07-13 — 1M context window now always-on (soft ~200K aim kept for WORK-UNIT + PLANNING; MILESTONE-REVIEW unbounded); upstream drop-in reframed `CLAUDE.md` + `/session-prompt`; `.agent/context.sh` already defaulted to 1M (untouched — its `CLAUDE_CODE_DISABLE_1M_CONTEXT`→200K branch is correct fallback, not stale). Recurring absorption gotcha: the `/session-prompt` drop-in keeps stripping the local `.claude/tech/*.md` nav hint (the "Read only what the step implicates" line) → restore from git each absorption (cf. Serena `ignored_paths` comment below).
- 2026-07-07 — Serena `project.yml` template rewrites (version updates that add keys) clobber the custom `ignored_paths` do-not-read sync comment — restore it from git, don't just "track as written" (the global-CLAUDE.md default); set values (`languages`/`ignored_paths`) carry over untouched. (`1d09009`)
