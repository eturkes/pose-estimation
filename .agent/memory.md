...
- `.scratch/census_3cam.py` + `.scratch/census_rig.py` — header-only corpus census over `videos/3-cam/` (cv2 container properties, no pixel decode; 4.5 s for 382 files). The first emits file/frame/duration totals, the `(w,h,fps,codec,rotation)` distribution, trial count and view coverage; the second adds rotation by view label, within-trial resolution/fps/frame-count agreement, and duration spread. **Every corpus number in `.agent/roadmap.md` M2 comes from these two**, so no M2 corpus claim reruns from committed state until M2.1 ports them into the committed inventory tool. `census_rig.py` imports `census_3cam` for `ROOT`/`parse` → run it with `PYTHONPATH=.scratch`. Its normalizer is deliberately partial (it drops 7 stems); M2.1 owns the exact one.
