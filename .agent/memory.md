...
- 2026-06-25 — In-container CPU/GPU/NPU OpenVINO accel verified (`available_devices == ['CPU','GPU','NPU']`; each compiles+infers correctly) and adopted as the **primary** run path; host-side `.venv-host` demoted to host-OS launches. Enablement is machine-local (Lunar Lake drivers + a `PYTHONPATH` accel-enabled OpenVINO runtime that precedes the pip wheel in `sys.path`), documented in the auto-injected, git-ignored `CLAUDE.local.md` — committed docs reference it without duplicating paths. The pip `openvino` dep stays (generic checkouts rely on it for CPU/GPU). **Where:** `environment.md` (Devices / inference, Host-side runs), `roadmap.md` (Backlog).
