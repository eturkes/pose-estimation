...
- `videos/` splits into `videos/initial/` (12 single-camera clips) + `videos/3-cam/` (three-view footage, no calibration). Filename-derived inventory + the discovery-contract mismatch → `.agent/roadmap.md` M2. Path-taking tools default to the old flat root: `scripts/run_report.py --videos-dir` and `pose-estimation-run --list-sessions` both need the subdirectory now.
