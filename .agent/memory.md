...
- 2026-06-22 — Adopted multi-mode session flow in `/session-prompt`: MODE auto-detected from the active milestone's status (UNPLANNED→PLANNING, IN-PROGRESS→WORK-UNIT, IMPLEMENTED→MILESTONE-REVIEW). Roadmap restructured into a milestone (M<m>) + unit (M<m>.<u>) ledger. PLANNING always uses a dynamic workflow + web search and reads the prior milestone's commit range to right-size units; WORK-UNIT records context-usage per unit; MILESTONE-REVIEW reserves the 1M context window. Commit trace-key in parens (`(M2.3)`/`(M2 plan)`/`(M2 review)`); codex-review follow-ups add a `Codex-Review:` trailer. Footage gate fixed: real path is the `videos/` symlink (not `rehab/data/videos`) and the condition is "no real 3-camera session", not "no footage" (single-cam clips exist for the 2D path). M2 (Phase 2) left UNPLANNED + footage-gated — unit-planning deferred until a real 3-cam session exists (user choice; footage expected, timing unknown). This session = methodology adoption only; no workflow run.
