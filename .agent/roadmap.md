...
**Open defect, M3.3a, fix before M3.3b widens the artifact.** `n_expected_frames` measures the span of the window's *observed* samples, not the half-open window P03 specifies. `trajectory_grid()` anchors the grid on the first observed sample (`analysis/clinical_features.R:455`, `raw <- (t - t[1]) * fs`, `n_grid <- slot[length(slot)] + 1`), so rows physically absent at a window edge fall outside the grid: a window that lost its leading frames reports `frame_coverage = 1.0`. Interior loss stays exact, which is why the corpus missed it — every fixture deletes interior rows only. Reported by `rev-m3u3a-1`, confirmed by construction.

Fix shape: pad the validity mask to the window's nominal slot count before `grid_evidence()`, leaving `trajectory_metrics()`'s estimate arithmetic on the grid it uses today — widening the estimate grid would move `nj` through `T_dur` and break P08. Edge runs then count as gaps, which V15 already requires. Acceptance: a fixture whose window loses its first three rows reports `n_expected_frames` = the full nominal count, `frame_coverage` = the reduced ratio, `longest_gap_frames` = 3, `n_gaps` = 1, with every gap-free golden byte-identical.

Second report from the same reviewer, unconfirmed by MAIN: a fully reversed clip emits zero QC rows, which would break V21-V24's requirement that an unusable timebase still produce keyed rows. Reproduce before acting.
