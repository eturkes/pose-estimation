...
3. Anonymization strategy section (in docs/capture_protocol.md; also record a decision entry): raw patient video is never committed (videos/ is already gitignored); shareable fixtures are de-identified derived artifacts only — per-camera keypoint CSVs + calibration.json + world3d.csv (coordinates, no imagery); any retained imagery must be synthetic (ChArUco board / synthetic skeleton render), never patient frames; strip capture metadata; derive fixtures only from consented recordings.
