...
Load `.agent/roadmap.md` (milestone ledger + active-milestone detail), then `.agent/memory.md` (lessons + decisions); CLAUDE.md (imports `AGENTS.md`) is auto-injected. Read only what the step implicates — subsystem notes under `.claude/tech/*.md` (filenames index them). Navigate via tokensave or LSP where available, else grep.
