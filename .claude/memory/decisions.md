...
## 2026-06-16 — CLAUDE.md realignment: Serena `ignored_paths` ↔ non-gitignored `Read()`-deny sync

**Context.** User edited `/CLAUDE.md` (no code change) and asked to bring the project into alignment. The diff is overwhelmingly rephrasing + factualised references (URLs → local `~/agents/docs/*` + `~/.local/bin/bgcmd`, all verified present) and now-true statements ("maintain the *established* memory/roadmap system", `/session-prompt` formalised). One clause is genuinely new and actionable: a "do-not-read" set distinct from `.gitignore`, enforced via `permissions.deny` `Read()` rules, with the explicit mandate to "add any non-gitignored 'do-not-read' entry to Serena's `ignored_paths` and keep those surfaces in sync." `.serena/project.yml` shipped `ignored_paths: []`, so committed do-not-read files were hidden from the `Read` tool/Bash but still surfaced by Serena's file-search tools.
**Decision.** Populate `ignored_paths` with exactly the **non-gitignored** entries of `.claude/settings.json`'s `Read()`-deny list: `LICENSE`, `uv.lock`, `renv.lock`, `renv/`. The invariant going forward: a deny entry that is *also* gitignored needs no mirror (Serena's `ignore_all_files_in_gitignore: true` covers it); a deny entry that is *committed/non-gitignored* must be mirrored into `ignored_paths`. `.git/**` is deliberately excluded (Serena traversal-excludes it intrinsically; mirroring would be noise). All other CLAUDE.md changes were already satisfied: `.serena/memories/` is gitignored via the broader `.serena/*` glob (no redundant explicit line added); `.serena/project.yml` is tracked.
**Alternatives considered.** (a) Mirror the *entire* deny list into `ignored_paths` — rejected: the gitignored majority is already honored by Serena, so mirroring them duplicates state that will drift. (b) Add an explicit `.serena/memories/` gitignore line to match CLAUDE.md's wording verbatim — rejected: `.serena/*` is strictly broader and already correct; a redundant line invites contradiction. (c) Record nothing (rule already lives in CLAUDE.md) — rejected: the *current mapping* and the `.git` exclusion rationale are the reusable artifact; CLAUDE.md states the rule, this entry states its application.
**Consequences.** Any future session that adds a `Read()`-deny rule must classify it: gitignored → done; committed → also append to `project.yml` `ignored_paths`. Removing a committed do-not-read deny rule means removing its `ignored_paths` mirror too. Reverse by restoring `ignored_paths: []`.
**References.** `.serena/project.yml` (`ignored_paths`), `.claude/settings.json` (`permissions.deny`), `.gitignore` (`.serena/*` block), `/CLAUDE.md` (do-not-read clause), `.claude/INDEX.md` (settings.json deny-behaviour line).

---

## 2026-06-15 — Capture QA gate + protocol + anonymization (1C)

**Context.** Roadmap Session 1C: a human capture/QA protocol for real 3-camera recording, an automated gate that grades a *raw capture* before its clinical metrics are trusted, and the anonymized-fixture strategy. Hard questions: which capture faults are cheaply detectable pre-fusion; how to measure calibration-board "coverage" when the solver exposes no per-frame poses; where capture-QA thresholds live relative to the 1B report thresholds; and what may ever be committed from patient recordings.
**Decision.** New `qa_check(session_dir, *, calibration, output_dir, camera_processor, device, backend, confidence_floor, board) -> QAReport` in `validation.py`, **reusing** the 1B grading vocabulary (`Band`/`Check`/`Grade`/`Verdict`) and harness blocks (`detect_charuco_corners`, `_resolve_external_calibration`, `_run_tracking` + `_measure_tracking`) — zero new pipeline maths. It grades three pre-fusion failure surfaces: (1) **calibration capture** — solved/loaded reprojection RMS + per-camera ChArUco detection count/rate + **board FOV coverage** = grid-occupancy of pooled board corners over an 8×6 image grid (`COVERAGE_GRID`), chosen over per-frame solvePnP pose-spread because the solver exposes no poses and coverage directly encodes the 2026-06-08 "narrow pose cloud → weak intrinsics" lesson at detection cost only; (2) **synchronization** — raw frame-count parity `(max−min)/max` as a software-sync desync proxy (no genlock); (3) **subject clip** — per-camera 2D detection rate via the same three-way tracking source as `run_validation`, *degrading to unassessed* rather than raising. Thresholds: a separate versioned `QA_THRESHOLDS` (`QA_THRESHOLDS_VERSION`, independent of report versions) for capture-specific bands (charuco frame floor = `MIN_INTRINSIC_FRAMES`, detection rate, coverage, parity, subject detection), **sharing** `THRESHOLDS.calib_reproj_rms_px` + `confidence_floor` so the physical quantities keep one definition; all new bands flagged **provisional** (Session 2A re-calibrates). CLI: `--qa-only` branches `main` to `qa_check`; both report types share a duck-typed `_emit` (JSON + Markdown + verdict + exit code 0 PASS/WARN, 1 FAIL, 2 error, `--strict` WARN→1). Added `video_io.frame_count` (CAP_PROP_FRAME_COUNT, grab-count fallback) so `validation.py` stays free of direct cv2. **Anonymization** (full text in `docs/capture_protocol.md`): raw patient video + `output/` never committed (already gitignored); calibration is patient-adjacent and gitignored at any depth (decision 2026-06-15), so a committed de-identified fixture needs a scoped `!` negation or `git add -f`; shareable fixtures are **de-identified derived coordinates only** (per-camera keypoint CSVs + `calibration.json` + `world3d.csv` — no imagery); any retained imagery must be synthetic (ChArUco/synthetic skeleton), never patient frames; strip capture metadata; derive only from consented recordings.
**Alternatives considered.** (a) Fold QA bands into `THRESHOLDS` — rejected: report-grading and capture-grading are distinct lifecycle stages with independent version cadences; only the genuinely shared physical bands are reused. (b) Board coverage via per-frame solvePnP translation+tilt spread — rejected: the solver returns no per-frame poses, re-deriving them couples QA to pose maths, and RMS already penalises poor tilt diversity; grid-occupancy is cheaper and directly interpretable ("board swept N % of the frame"). Tilt diversity stays protocol-enforced + RMS-caught. (c) `qa_check` raising on a missing subject backend — rejected: a pre-flight gate must grade whatever is present and surface the rest as notes, never abort. (d) Import cv2 into `validation.py` for frame counting — rejected: a `video_io.frame_count` helper keeps the orchestration module cv2-free and is independently reusable/testable. (e) Make the synthetic good-capture a clean PASS by widening the board sweep — rejected: the oblique synthetic cameras honestly land at coverage WARN; the test asserts not-FAIL + every sufficiency check PASS, which is the truthful contract.
**Consequences.** Session 2A re-tunes the provisional QA bands against the first real capture (record deltas, bump `QA_THRESHOLDS_VERSION`). Session 2C's anonymized-fixture step follows the `docs/capture_protocol.md` strategy verbatim and must add any committed `calibration.json` fixture via a scoped negation / `-f`. Adding a QA metric = one `QAThresholds` field + one `_grade_qa` check + a `validation.md` table row. The world-frame-level and synchronization gap-register rows now cite the protocol + QA proxy as partial mitigations (sub-frame desync + a gravity reference remain open). Reverse by deleting the QA section in `validation.py`, the `--qa-only` flag, `frame_count`, `docs/capture_protocol.md`, and the QA tests.
**References.** `src/pose_estimation/validation.py` (`qa_check`, `QAThresholds`/`QA_THRESHOLDS`, `QAReport` tree, `_qa_calibration`/`_charuco_camera_qa`/`_board_coverage`/`_qa_parity`/`_qa_subject`/`_grade_qa`, `_emit`, `main`), `src/pose_estimation/video_io.py` (`frame_count`), `docs/capture_protocol.md`, `tests/test_validation.py` (QA suite), `tests/test_helpers.py` (`frame_count`), `.claude/tech/validation.md` (QA gate + thresholds + gap register), `.claude/prompts/sessions.md` (Session 1C).

---

## 2026-06-15 — Validation thresholds + verdict (1B): graded self-consistency, informational perf/symmetry

**Context.** Roadmap Session 1B: turn 1A's raw `ValidationReport` into a graded PASS/WARN/FAIL verdict with documented, literature-grounded thresholds plus an honest clinical-validity gap register. The hard questions: which metrics actually drive a *clinical* verdict (vs. perf noise), how to grade meaningfully when no ground-truth baseline exists, and what a single agreement tolerance can honestly cover across heterogeneous units.
**Decision.** `THRESHOLDS` is one frozen versioned `Thresholds` dataclass (single source of truth; folds in `confidence_floor`, so `DEFAULT_CONFIDENCE_FLOOR` now derives from it). `THRESHOLDS_VERSION` is independent of `REPORT_SCHEMA_VERSION` (bumped 1→2 for the `verdict` JSON block; threshold *value* changes bump only `THRESHOLDS_VERSION`). Each metric is a `Band(warn, fail, direction)`; non-finite → WARN (surfaced, never silently passed). `ValidationReport.verdict()` grades every metric; overall = worst **non-informational** check. **Extended the task's explicit threshold list** with two universally-valid self-consistency surrogates — bone-length CoV and temporal jitter — *because* with no baseline (decision 2026-06-15) they are the only gradeable clinical evidence, and both hold on any input (rigid bones, smooth motion). **Timing throughput and L/R symmetry are graded but informational** (excluded from the overall): throughput conflates perf with validity and its denominator includes one-time solve/R (a slow-but-correct reconstruction is still valid); symmetry is valid only on symmetric-by-construction input (real anatomical asymmetry would false-FAIL a subject). Baseline agreement grades only `_deg` metrics against one well-grounded clinical tolerance (~5°); non-angle metrics are noted and deferred to 2C's unit-aware Bland-Altman/ICC. CLI exit: 0 PASS/WARN, 1 FAIL, 2 harness error; `--strict` promotes WARN→1 for CI. Literature: calib RMS warn 1/fail 2 px (Pose2Sim, Pagnon 2021); fusion reproj median 8/12, p95 15/20 px (Anipose <12 px@75 %, >20 px=missing; Pose2Sim ~10 px); angle 5/10° (OpenCap ~6° borderline). Low-conf / unfused / throughput bands are flagged **provisional** — Session 2A re-calibrates against the first real capture.
**Alternatives considered.** (a) Grade symmetry + throughput into the overall — rejected: false-FAILs real asymmetric subjects and slow machines, and would make the synthetic E2E flap on machine speed. (b) One scalar agreement tolerance across all metrics — rejected: degrees and metres are not comparable; grade only `_deg` now, unit-aware later. (c) Store verdict as a dataclass field computed at construction — rejected: `verdict(thresholds=…)` must accept overrides; compute on demand and merge into `to_json`/`to_markdown`. (d) WARN→nonzero by default — rejected: WARN = "usable with caveats" (exit 0); offered via `--strict`. (e) Honour the explicit list verbatim (no bone-CV/jitter grading) — rejected: the no-baseline verdict would be near-vacuous until Phase 2.
**Consequences.** Session 2A re-tunes the provisional bands (record the deltas as a new decision; bump `THRESHOLDS_VERSION`). Session 2C adds a baseline + unit-aware agreement (Bland-Altman/ICC), replacing the `_deg`-only stopgap. Adding a metric = one `Thresholds` field + one `_grade_report` check + a `validation.md` table row. The gap register in `validation.md` is the standing honesty ledger (agreement UNVALIDATED is the headline). Reverse by deleting the grading section, `verdict()`, the `--strict` flag, and reverting `REPORT_SCHEMA_VERSION` to 1.
**References.** `src/pose_estimation/validation.py` (`Grade`/`Band`/`Thresholds`/`THRESHOLDS`, `_grade_report:1010`, `ValidationReport.verdict`, `main:1300`), `tests/test_validation.py` (verdict suite + `_good_report`), `.claude/tech/validation.md` (thresholds table + gap register), `.claude/tech/optimization.md` (tunables↔thresholds), `prompts/sessions.md` (Session 1B).

---

## 2026-06-15 — Validation harness (1A): orchestrate-and-measure, baseline-optional surrogates

**Context.** Roadmap Session 1A: build a reproducible end-to-end validation harness (`validation.py` + `pose-estimation-validate`) running the full 3D clinical chain on one session and emitting a structured `ValidationReport`. Footage-independent — validated on synthetic data. The hard design questions: how to reuse the pipeline without reimplementing maths yet still get per-stage timing; what to report when no ground-truth baseline exists.
**Decision.** The harness **orchestrates and measures only** — zero pipeline maths. It runs the substantive blocks individually (`solve_charuco`, `fuse_session_outputs` + `write_world3d_csv`, `read_csv_keypoints`, R `clinical_features.R`) rather than via `process_session`, *because* `process_session` bundles tracking+fusion under one call and would deny each stage its own wall-clock. 2D tracking is a three-way branch (injected `camera_processor` for tests / reuse ≥2 existing CSVs / subprocess `python -m pose_estimation.run --headless`). Agreement is baseline-optional (per the 2026-06-15 seed): with a baseline → per-metric error; without → self-consistency surrogates computed straight from `world3d.csv` — bone-length CoV, L/R symmetry relative diff, and temporal jitter via the **second** temporal difference (acceleration isolates frame noise from steady movement). `unfused_keypoint_fraction` is measured over **active** keypoints (fused ≥1 frame) only; a never-tracked keypoint is a 2D gap (surfaced in `tracking_2d.detection_rate`), not a fusion failure. Report carries **raw numbers only** — no PASS/WARN/FAIL.
**Alternatives considered.** (a) Reuse `process_session` wholesale — rejected: no per-stage timing, and it would re-run tracking the harness wants to inject/skip. (b) Reimplement triangulation/clinical maths for tighter measurement hooks — rejected: violates single-source-of-truth, invites drift. (c) Unfused fraction over the full skeleton — rejected: legs/face in arm mode inflate it meaninglessly. (d) Plain first-difference (velocity) jitter — rejected: penalises legitimate steady movement; second difference is movement-robust. (e) Emit a verdict now — rejected: thresholds are Session 1B's deliverable.
**Consequences.** Session 1B layers versioned thresholds + PASS/WARN/FAIL on top of this raw schema (bump `REPORT_SCHEMA_VERSION` only on layout change, not threshold change). `_BONES`/`_SYMMETRIC_BONES` cover both arm- and body-mode names and self-select by endpoint presence; adding a tracking mode means extending those tables. Real-backend tracking (Phase 2) flows through the existing subprocess branch unchanged. Reverse by deleting `validation.py`, its console-script line, test, and tech note.
**References.** `src/pose_estimation/validation.py` (`run_validation:247`, `ValidationReport:218`, `_build_agreement:694`, `main:982`), `tests/test_validation.py`, `.claude/tech/validation.md`, `pyproject.toml` (`pose-estimation-validate`).

---

## 2026-06-15 — Default-deny `.gitignore` for calibration + credentials; history verified clean

**Context.** Task: ensure all sensitive information is gitignored and the Git history is clean. Audit found the history **already clean** — 122 commits across `main`/`origin/main`, every path ever committed is a source/config/doc type, largest blobs are `renv.lock`/`uv.lock`/`repomap.md` (no binary/video/CSV ever entered history), and secret-pattern scans over all historical blobs **and** all commit messages returned nothing real (only benign hits: the word "password" in `CLAUDE.md`/`renv.lock` descriptions and `../../etc/passwd` path-traversal *test fixtures*). Nothing sensitive is tracked or sitting untracked; all data dirs are already ignored.
**Decision.** No history rewrite (none warranted). Closed forward-looking `.gitignore` gaps with a default-deny posture: added `calibration/` + `calibration.json` (any depth) — fulfils the 2026-05-18 multicam TODO; calibration is patient-adjacent (identifying lab/rig geometry); added `.env` / `.env.*` / `*.env` (credential safety net — none used in-repo today); broadened `debug.log` → `*.log` (logs may capture session/patient paths). Verified every new pattern via `git check-ignore` and that no tracked file is masked.
**Alternatives considered.** (a) Root-anchor calibration (`/calibration.json` only) so a future fixture stays addable — rejected: leaves arbitrary `--calibration <path>` files git-addable; any-depth default-deny is safer for clinical data. (b) Rewrite history (BFG/filter-repo) — rejected: nothing sensitive is present, and a rewrite would force-push and invalidate 122 commits (remote is user-owned). (c) Skip `.env`/`*.log` as unused — rejected: cheap defense-in-depth for a patient-data project.
**Consequences.** A future de-identified `calibration.json` fixture (roadmap Session 1C) must use a scoped `!` negation or `git add -f` — flagged inline in `prompts/sessions.md` 1C. `videos`/`output` are symlinks; git refuses to traverse beyond them and the symlink names are themselves ignored, so patient data is doubly protected (a multi-arg `git check-ignore` aborts on the first symlinked path — probe such paths one at a time). Reverse by dropping the added `.gitignore` lines. Supersedes the gitignore-TODO clause in the 2026-05-18 multicam-scaffolding entry below.
**References.** `.gitignore`, `.claude/prompts/sessions.md` (Session 1C anonymization step), `src/pose_estimation/calibration.py` (`CALIBRATION_FILENAME`).

---

## 2026-06-15 — Seed next roadmap: Real-World 3D Clinical Validation (no build this session)

**Context.** Prior roadmap (Stability + Clinical Metrics + 3D Pipeline) has Phases 1–3 done but the entire 3D path is **synthetic-unit-tested only** — real 3-camera footage has never run end-to-end. Task: seed the next roadmap with real-world validation of the 3D clinical pipeline (reproducible harness reporting reprojection error, dropped/low-confidence keypoints, timing, clinical-metric agreement; plus capture/QA protocol, anonymized fixtures, failure-mode tests, thresholds, clinical-validity gaps). Three blockers resolved with the user up front: **footage = none yet**, **baseline = none yet**, **scope = roadmap only** (write the roadmap + per-session prompts; build nothing this session).
**Decision.** Seeded the "Real-World 3D Clinical Validation" roadmap in `prompts/sessions.md`: Phase 1 (footage-independent scaffolding, runnable now on synthetic data) = 1A harness core + report schema, 1B thresholds + clinical-validity gap register, 1C capture/QA protocol + automated QA gate + anonymization strategy, 1D failure-mode tests; Phase 2 (gated on footage; agreement also gated on a baseline) = 2A first real-capture dry run, 2B quantitative real-data validation, 2C clinical-metric agreement + anonymized real fixtures + regression lock; Phase 3 = shared Maintenance cycle. Archived the completed roadmap to a status checklist and **pruned its now-obsolete per-session prompt blocks** (recoverable from Git history); retained the roadmap-agnostic Maintenance prompt. Built no code/docs/tests per the roadmap-only scope.
**Alternatives considered.** (a) Build the footage-independent foundation now (harness + protocol + fixtures + tests + tech note) and seed the roadmap — offered; user chose roadmap-only. (b) Build + run on real data this session — impossible (no footage). (c) Keep the completed roadmap's detailed prompt blocks — rejected as bloat; Git history preserves them. (d) One mega "validation" session — rejected; decomposed into fresh-agent-sized sessions per CLAUDE.md.
**Consequences.** A no-arg `/session-prompt` picks up **1A immediately** (footage-independent). Phase 2 agents must **stop and report the block** if `rehab/data/videos` still has no real session. The agreement leg is designed baseline-optional: absent a baseline, validation rests on reprojection error + cross-camera consistency + temporal stability + inter-trial repeatability — and "clinical-metric agreement vs ground truth: UNVALIDATED (no baseline)" is the headline standing gap to register in 1B. Future sessions will create `validation.py`, `pose-estimation-validate` console script, `.claude/tech/validation.md`, and `docs/capture_protocol.md` — each must update INDEX.md / tests.md / repomap in the same change (drift rule). Reverse by restoring the prior `sessions.md` from Git.
**References.** `.claude/prompts/sessions.md` (Current roadmap + Sessions 1A–2C + Maintenance), `.claude/memory/decisions.md` (this entry), supersedes the "real-footage validation … natural seed for the next roadmap" note in the 2026-05-24 roadmap entry below.
