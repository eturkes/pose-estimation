...
├── commands/         # project slash commands (/session bootstraps a session)
...
| Command | Use |
|---------|-----|
| `/session` (`commands/session.md`) | Bootstraps a fresh session. `/session <TASK>` overrides the roadmap; `/session` alone continues it. Replaces the former `prompts/kickoff.md`. |

## prompts/

| File | Use |
|------|-----|
| `prompts/sessions.md` | Roadmap status + per-task session prompts. The `/session` command reads this to continue the roadmap when no task is given. |
