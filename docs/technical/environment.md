...
- **Primary path = in-container, and it needs the accelerator environment sourced first.** That script supplies the Intel GPU/NPU userspace paths (`LD_LIBRARY_PATH`, OpenCL ICD, Level Zero, accelerator `PYTHONPATH`) and selects the OpenVINO build matching the container's C library. Source it before Python starts, because the loader reads `LD_LIBRARY_PATH` at process startup. On a two-layer machine the *inherited* `PYTHONPATH` can point at the host-side build instead, which fails loudly rather than degrading: `import openvino` raises `ImportError: /lib/x86_64-linux-gnu/libm.so.6: version 'GLIBC_2.43' not found`. With no accelerator environment at all, the `.venv` pip `openvino` wheel serves CPU alone.
